...
const Navbar = () => {
  ...
  return (
    <div>
      {
        user._id ? (
          <div className="bg-yellow-600 bg-opacity-50 text-white text-xs text-center uppercase tracking-widest py-2 flex justify-center">
            
            <NavLink to={`/user/${user?._id}`} >
              <p>Welcome to <span className='underline'>{user?.firstName}</span></p>
            </NavLink>

            <NavLink className="mx-5" to = '/' onClick={() => dispatch(logoutUser())}>
              Logout
              </NavLink>
          </div>
        ) : (
